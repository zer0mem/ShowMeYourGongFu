		...
		)
	{
		...
		if (img_base)
		{
			...
			if (mz.IsValid())
			{
				...
				const IMAGE_DATA_DIRECTORY* img_dir = reinterpret_cast<const IMAGE_DATA_DIRECTORY*>(img_dir_map.ReadPtrUser());
				if (img_dir)
				{
					...
					const IMAGE_EXPORT_DIRECTORY* export_dir = reinterpret_cast<const IMAGE_EXPORT_DIRECTORY*>(export_dir_map.ReadPtrUser());
					if (export_dir)
