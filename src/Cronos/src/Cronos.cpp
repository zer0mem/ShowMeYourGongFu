...
void HVCallback4(__inout ULONG_PTR reg[REG_COUNT]);
...
CCRonos::CCRonos()
{
	...
	m_vCpu = reinterpret_cast<CVirtualizedCpu*>(malloc(sizeof(CVirtualizedCpu) * cpu_count));
	if (m_vCpu)
	{
		RtlZeroMemory(m_traps,sizeof(m_traps));

		m_callbacks = reinterpret_cast<CALLBACK*>(malloc(sizeof(*m_callbacks)));
		if (m_callbacks)
		{
	...
	)
{
	...
	m_vCpuCount++;
	::new(m_vCpu + coreId) CVirtualizedCpu(coreId, m_traps, !!m_traps[VMX_EXIT_EXCEPTION], HVCallback, m_callbacks);
}
...
bool CCRonos::RegisterCallback( __in CALLBACK* callbacks, __in const void* callback )
{
	...
	CALLBACK* t_callback = reinterpret_cast<CALLBACK*>(malloc(sizeof(CALLBACK)));
	if (!t_callback)
		return false;
...
void CCRonos::HVCallback(__inout ULONG_PTR reg[REG_COUNT], __in_opt CALLBACK** callbacks)
{
	if (!callbacks)
		return;
...
void CCRonos::HVCpuid( __inout ULONG_PTR reg[REG_COUNT] )
{
	reg[RAX] = kCpuidMark;
}
